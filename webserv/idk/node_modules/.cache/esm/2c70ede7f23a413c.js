let SerialPort,StringStream,mqtt,env,admin,spawn;_f23‍.w("serialport",[["default",["SerialPort"],function(v){SerialPort=v}]]);_f23‍.w("scramjet",[["StringStream",["StringStream"],function(v){StringStream=v}]]);_f23‍.w("mqtt",[["default",["mqtt"],function(v){mqtt=v}]]);_f23‍.w("./env.json",[["default",["env"],function(v){env=v}]]);_f23‍.w("firebase-admin",[["default",["admin"],function(v){admin=v}]]);_f23‍.w("child_process",[["spawn",["spawn"],function(v){spawn=v}]]);// ESM syntax is supported.
...
const thresholdListener = () => {
    admin.database().ref('MQTT/' + env.bridgeId + '/thresholds/concentrator_1/').on('value', (snapshot) => {
        ...
        _f23‍.g.console.log(thresholdValue)
    })
    admin.firestore().collection("Bridge").doc(env.bridgeId)
        .onSnapshot((doc) => {
            ...
            _f23‍.g.console.log(bridgeMetaData)
        })
}
        ...
        admin.firestore().collection('notification_history').add({
            ...
            if (message) {
                admin.messaging().send(message)
                    .then(() => {
                        _f23‍.g.console.log('Notification sent!', timestamp);
                    })
                    .catch((error) => {
                        _f23‍.g.console.log('Error sending message:', error);
                    });
            }
        })
    }
            ...
            SerialPort.list().then(ports => {
                ports.forEach((port, i) => {
                    ...
                    var handler = new SerialPort(port.path, {
                        baudRate: 57600
                    }, error => error ? _f23‍.g.console.log(error.toString()) : '')
                    var failedCOunt = 0
                    var norepCount = 0
                    handler.pipe(parser).lines().each(data => {
                        ...
                        if (temp[1] === 'Ready') {
                            ...
                            failedCOunt += 1
                            _f23‍.g.console.log('RTU' + RTUId, timestamp, data, '-', failedCOunt)
                        } else if (temp[1] === 'no') {
                            norepCount += 1
                            _f23‍.g.console.log('RTU' + RTUId, timestamp, data, '-', norepCount)
                        } else {
                            if (temp) {
                                temp.splice(0, 3)
                                temp.map((value) => {
                                ...
                                _f23‍.g.console.log('RTU' + RTUId, timestamp, 'length of data: ', value.length)
                                ...
                                switch (RTUId) {
                                    case '01':
                                        if (value.length >= 100) {
                                            ...
                                            _f23‍.g.console.log('RTU' + RTUId, timestamp, 'Data error! Either sendToWait failed or no reply on either X or Y.')
                                        }
                                        break;
                                    case '02':
